...
public class Testing {
    ...
    // If logout is after 5 PM, limit to 5 PM
    if (logout.isAfter(officialEnd)) {
        logout = officialEnd;
    }

    // If login is before 8 AM, make it 8 AM
    if (login.isBefore(officialStart)) {
        login = officialStart;
    }

    // Apply grace period (8:00–8:10 counts as 8:00)
    if (!login.isAfter(graceTime)) {
        login = officialStart;
    }

    // If login is already after logout, return 0
    ...
    for (int month = 6; month <= 12; month++) {
        ...
        double firstGrossSalary = firstHalf * hourlyRate;
        double secondGrossSalary = secondHalf * hourlyRate;
